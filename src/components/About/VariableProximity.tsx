...
const VariableProximity = forwardRef<HTMLSpanElement, VariableProximityProps>((props, ref) => {
    ...
    }, [containerRef, parsedSettings, radius, calculateFalloff, fromFontVariationSettings, mousePositionRef]);

    useAnimationFrame(animationCallback);

    const words = label.split(" ");
    letterRefs.current = []; // Reset refs array each render
    let letterIndex = 0;
            ...
            {words.map((word, wordIndex) => (
                ...
                >
                    {word.split("").map((letter) => {
                        ...
                        return (
                            <motion.span
                                key={`${wordIndex}-${currentLetterIndex}`}
                                ref={(el) => { 
